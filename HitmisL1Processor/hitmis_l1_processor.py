...
parser.add_argument('prefix',
                    nargs='?',
                    default=None,
                    help='Prefix of final L1 data file name. If ')
...
# intialize model
predictor = HMS_ImagePredictor(hmsVersion=config)
if prefix is None:
...
for wl in wlarr_nm:
    ...
    while current_start_time <= end_date:
        ...
        current_files = [file for file in flist if current_start_time <=
                        datetime.fromtimestamp(getctime(file)) <= current_end_time]
